...
def set_brightness(level: int):
    if 0 <= level <= 100:
        ...
        return f"🔆 Brightness set to {level}%"
    return "⚠️ Brightness must be between 0-100."

def set_volume(level: int):
    os.system(f"pactl set-sink-volume @DEFAULT_SINK@ {level}%")
    return f"🔊 Volume set to {level}%"

def toggle_wifi(status: str):
    if status.lower() in ["on", "off"]:
        os.system(f"nmcli radio wifi {status}")
        return f"📶 Wi-Fi turned {status}."
    return "⚠️ Use 'on' or 'off'."

def toggle_dark_mode(status: str):
    theme = "Adwaita-dark" if status.lower() == "on" else "Adwaita"
    os.system(f"gsettings set org.gnome.desktop.interface gtk-theme {theme}")
    return f"🌙 Dark mode turned {status}."
...
def open_application(app_name: str):
    ...
    if command:
        subprocess.Popen(command, shell=True)
        return f"📂 Opening {app_name}..."
    return f"⚠️ Application '{app_name}' not supported."
...
def recommend_music(mood: str):
    ...
    try:
        playlist = sp.playlist(playlist_id, market="US")
        return f"🎵 Listen to: [{playlist['name']}]({playlist['external_urls']['spotify']})"
    except:
        return f"🎵 Listen here: [Spotify Playlist](https://open.spotify.com/playlist/{playlist_id})"
...
def send_sms(phone: str, message: str):
    ...
    try:
        ...
        return f"❌ SMS failed. Status: {response.status_code}"
    except requests.exceptions.RequestException as e:
        return f"⚠️ Error: {e}"
...
if user_input:
    ...
    # --- Detect SMS Request Naturally ---
    sms_match = re.search(r'(?:send|text|message)\s+(?:an\s+)?sms\s*(?:to\s*)?(\+?\d{10,15})[:,]?\s*(.*)', user_input, re.IGNORECASE)

    if sms_match:
        phone, message = sms_match.groups()
        ai_response = send_sms(phone.strip(), message.strip())

    elif "open" in user_input.lower():
        app_name = user_input.split("open")[-1].strip()
        ai_response = open_application(app_name)

    elif "brightness" in user_input.lower():
        brightness_level = int(re.findall(r'\d+', user_input)[0])
        ai_response = set_brightness(brightness_level)

    elif "volume" in user_input.lower():
        volume_level = int(re.findall(r'\d+', user_input)[0])
        ai_response = set_volume(volume_level)
